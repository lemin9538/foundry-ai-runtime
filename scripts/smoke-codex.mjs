...
if (!status.available) {
  throw new Error(`Codex CLI is not available: ${status.message ?? "unknown reason"}`);
}
...
const result = await generateObject({
  ...
  system: "Do not inspect files or run commands. Return only the requested structured data.",
  prompt: "Confirm that this structured-output smoke test succeeded. Use transport codex-cli and ok true.",
  timeoutMs,
  retry: { maxAttempts: 1 },
});
